...
public class VisionIOReal implements VisionIO {
  ...
  /** Horizontal error from center of limelight lens center to target Units: Radians */
  ...
  public VisionIOReal(String limelightName) {
    ...
    inst.addListener(
        pipelineLatencyEntrySub,
        EnumSet.of(NetworkTableEvent.Kind.kValueAll),
        networkTableEvent -> {
          // Compute in seconds
          var pipelineLatencies =
              (pipelineLatencyEntrySub.getAtomic().value + captureLatencyEntrySub.getAtomic().value)
                  / 1.0e3;
