...
public class FlywheelIOSim implements FlywheelIO { //FlywheelIOSim makes Advantage kit log simulated movements using physics 
  private FlywheelSim sim = new FlywheelSim(DCMotor.getKrakenX60(2), FlywheelConstants.GEAR_RATIO, 0.004); //initiates virtual
  ...
  public void updateInputs(FlywheelIOInputs inputs) {
    appliedVolts =
        MathUtil.clamp(pid.calculate(sim.getAngularVelocityRPM() / 60) + feedFoward.calculate(sim.getAngularVelocityRPM() / 60), FlywheelConstants.LOW_VOLTAGE_BOUNDARY, FlywheelConstants.HIGH_VOLTAGE_BOUNDARY); //calculates from virutal motor's velocity from RPM to RPS
 ...
 */
  ...
  public void setVelocity(double velocityRPM) { //ffvolts is feedorward
    sim.setState(pid.calculate(velocityRPM / 60.0));
  }
