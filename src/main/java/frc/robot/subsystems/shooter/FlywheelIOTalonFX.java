...
public class FlywheelIOTalonFX implements FlywheelIO { // FlywheelIOTalonFX makes Advantagekit log physical hardware movement
  private final TalonFX leftFlywheelMotor = new TalonFX(ShooterConstants.LEFT_FLYWHEEL_MOTOR_ID); // Leader=left motor
  private final TalonFX rightFlywheelMotor = new TalonFX(ShooterConstants.RIGHT_FLYWHEEL_MOTOR_ID); // follower = right motor
  ...
  private final StatusSignal<Double> followerCurrent = rightFlywheelMotor.getSupplyCurrent(); // All of these are from TalonFX Phoenix 6 that assign values to advantagekit log variables
  ...
  public FlywheelIOTalonFX() {  // Object to set different flywheel configs 
    ...
    leftFlywheelMotor.getConfigurator().apply(flywheelConfig); // Apply the flywheel config defined above
    ...
    rollerConfiguration.MotorOutput.DutyCycleNeutralDeadband = HardwareConstants.MIN_FALCON_DEADBAND;
    ...
    leftFlywheelMotor.optimizeBusUtilization(); // updates frequency based on how often the bus communicates with the motor
